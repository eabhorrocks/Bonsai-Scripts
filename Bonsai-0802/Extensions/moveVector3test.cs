...
public class moveVector3test
{
    ...
    public IObservable<Vector3[]> Process(IObservable<Vector3[]> source)
    {
        ...
        {
            result[i] = result[i] + Velocity; // just add the velocity vector
        }
        ...
        for (int i = nCoh; i < nDots; i++)
        {
            ...
            float y = (float)rng.Next(0,100)-50f;
    ...
    private void wrapAround(Vector3[] dotPos, int nDots, float top, float bottom, float left, float right, float front, float back)
    {
        ...
        // x border
        if(dotPos[i].X > right) {dotPos[i].X = left+0.05f;}
        else if(dotPos[i].X < left) {dotPos[i].X = right-0.05f;}
        // y border
        if(dotPos[i].Y > top+1){dotPos[i].Y = bottom-0.05f;}
        else if(dotPos[i].Y < bottom) {dotPos[i].Y = top+0.05f;}
        // z border
        if(dotPos[i].Z > front){dotPos[i].Z = back-0.05f;}
        else if(dotPos[i].Z < back) {dotPos[i].Z = front+0.05f;}
        }
    }
}
