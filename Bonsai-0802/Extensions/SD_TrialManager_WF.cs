...
public class SD_TrialManager_WF
{
    ...
    public IObservable<Tuple<Tuple<double, double>,Tuple<int, int, int, int>>> Process(IObservable<Unit> source)
    {
        return source.Select(value => 
        {
            ...
            float meanSpeed = 0f;
            int rightFaster = 1;
            double left_speed = 0f;
            double right_speed = 0f;
            ...
            for (int i=0; i<nRatios; i++) // loop through ratios until one is picked
            {
                ...
                {
                    ratioIndexToTest = i; 
                    speedRatio = RatiosList[meanSpeedIndexToTest][i];
                    break;
                }
            }
            ////////// BIAS CORRECTION //////////
            ...
            if (biasrnd <= pRight) // compare to pre-calculated pRight value (probability of right faster) 
            {
                ...
                if (rng.NextDouble() <= 0.5) // right speed is standard speed
                {
                    right_speed = meanSpeed;
                    left_speed = meanSpeed - speedRatio*meanSpeed;
                }
                else //left speed is standard speed
                {
                    left_speed = meanSpeed;
                    right_speed = meanSpeed + speedRatio*meanSpeed;
                }
            }
            else // 
            {
            ...
            var speeds = new Tuple<double, double>(left_speed, right_speed);
